...
function canShowEstimatedMMR(data) {
  let count = 0;
  if (data.games.p >= 5) count++;
  if (data.games.t >= 5) count++;
  if (data.games.z >= 5) count++;
  return count >= 2;
}
...
function switchChart() {
  ...
  let index = CHART_MODES.indexOf(current);

  let next;

  do {
    index = (index + 1) % CHART_MODES.length;
    next = CHART_MODES[index];

    
    if (next === "estimatedmmr" && !canShowEstimatedMMR(cachedData)) {
      continue;
    }
...
function updateChartIndicator(mode) {
  ...
  const effectiveModes = canShowEstimatedMMR(cachedData)
    ? CHART_MODES
    : CHART_MODES.filter(m => m !== "estimatedmmr");

  const index = effectiveModes.indexOf(mode) + 1;
  el.textContent = `(${index}/${effectiveModes.length})`;
}
...
function drawEstimatedMMRChart(data) {
  ...
  // Build array with game counts included
  const arr = [
    {
      key: "p",
      label: "Protoss",
  ...
  // 1. Filter races with enough games
  const eligible = arr.filter(r => r.games >= MIN_GAMES);

  // 2. Require at least two eligible races
  if (eligible.length < 2) {
    return; // silently skip MMR chart
  }

  // 3. Sort by estimated MMR
